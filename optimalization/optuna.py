...
def objective(trial):
    param_grid = {
        'iterations': trial.suggest_int('iterations', 100, 200, 500),
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.05, 0.1),
        'l2_leaf_reg': trial.suggest_int('l2_leaf_reg', 1, 3, 5),
        'loss_fuction': trial.suggest_categorical('loss_function', ['RMSE', 'MAE', 'Poisson', 'MAPE']),
        # 'boostrap_type': ['MVS', 'Bayesian', 'Bernoulli'],
        'bagging_temperature': trial.suggest_float('bagging_temperature', 0.0, 0.5, 1, 10),
        # 'subsample': [0, 1],
        # 'sampling_frequency': ['PerTree', 'PerTreeLevel'],
        # 'sampling_unit': ['Object', 'Group'],
        # 'mvs_reg': [0, 0.5, 1],
        'random_strength': trial.suggest_float('random_strength', 0, 0.5, 1),
        'use_best_model': trial.suggest_bool('use_best_model', True, False),
        # 'best_model_min_tree': [0, 1, 2],
        'depth': trial.suggest_int('depth', 4, 6, 8),
        ...
        'od_pval': trial.suggest_float('od_pval',1e-5, 1e-10, 1e-20),
        'od_wait': trial.suggest_int('od_wait', 10, 20, 30),
    }
